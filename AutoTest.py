...
def flashDevice(version):
    ...
    bat = open("le_zl1_fastboot.sh", "r")
    batlist = bat.readlines()
    for line in batlist:
        if "read" in line:
            batlist.remove(line)
    bat.close()
    bat = open("le_zl1_fastboot.sh", "w")
    for line in batlist:
        bat.write(line)
    bat.close()
    ...
    subprocess.Popen("chmod 775 le_zl1_fastboot.sh", stdout=fdout, stderr=fdout, shell=True).communicate()
    cur_task = subprocess.Popen("./le_zl1_fastboot.sh", stdout=fdout, stderr=fdout, shell=True)
